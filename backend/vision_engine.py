    ...
    for obj_name in expected_objects:
        ...
        if mask is not None:
            ...
            for contour in contours:
                ...
                if area > 800: # Min pixel area to avoid noise
...
def analyze_images(ref_img_path, proof_img_path, expected_objects, output_proof_path, output_diff_path):
    ...
    ref_names = [d["name"] for d in ref_detections if d["name"] not in ["unknown obstacle"] and not d["name"].startswith("extra item")]
    proof_names = [d["name"] for d in proof_detections]
    
    # Filter proof names for checklist relevance
    detected_expected_names = [name for name in proof_names if name in expected_objects]
    
    # If ref_names is empty, default it to the checklist expected_objects
    if not ref_names:
        ref_names = expected_objects
        
    # Determine missing and new objects
    missing_objects = []
    for item in expected_objects:
        if item not in detected_expected_names:
